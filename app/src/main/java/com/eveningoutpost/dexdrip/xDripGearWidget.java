 ...
 */
public class xDripGearWidget extends AppWidgetProvider {

    public static final String TAG = "xDripGearWidget";
    ...
    private static void updateAppWidget(Context context, AppWidgetManager appWidgetManager, int appWidgetId) {
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.x_drip_gear_widget);
        Log.d(TAG, "Update widget signal received");

        //Add behaviour: open xDrip on click
        Intent intent = new Intent(context, Home.class);
        PendingIntent pendingIntent = PendingIntent.getActivity(context, 0, intent, 0);
        views.setOnClickPendingIntent(R.id.xDripGearWidget, pendingIntent);
...
//                estimate = lastBgreading.calculated_value;
                ...
                if (timeAgo == 1) {
                    views.setTextViewText(R.id.readingAge, timeAgo + xdrip.getAppContext().getString(R.string.space_minute_ago) + extrastring);
                } else {
                    views.setTextViewText(R.id.readingAge, timeAgo + xdrip.getAppContext().getString(R.string.space_minutes_ago ) + extrastring);
                }
                if (timeAgo > 15) {
                ...
                views.setTextViewText(R.id.widgetTime, new SimpleDateFormat("HH:mm").format(new Date().getTime()));

                int bridgeBattery= Home.getPreferencesInt("bridge_battery", 0);

                views.setTextViewText(R.id.widgetxbattery, bridgeBattery + ((bridgeBattery < 200) ? "%" : "mV"));
                if (bridgeBattery < 50) views.setTextColor(R.id.widgetxbattery,Color.YELLOW);
                if (bridgeBattery < 25) views.setTextColor(R.id.widgetxbattery,Color.RED);
                else views.setTextColor(R.id.widgetxbattery,Color.GREEN);
                ...
                if ((low_occurs_at > 0) && (BgGraphBuilder.last_noise < BgGraphBuilder.NOISE_TOO_HIGH_FOR_PREDICT)) {
                    ...
                    android.util.Log.d(TAG, "evaluateLowPrediction: mins: " + predicted_low_in_mins);
                    if (predicted_low_in_mins > 1) {
                        if (predicted_low_in_mins < 90) {
                            views.setTextViewText(R.id.widgetLowPredict, xdrip.getAppContext().getString(R.string.low_predicted)
                                    +" "+ xdrip.getAppContext().getString(R.string.in)+" " + (int) predicted_low_in_mins + xdrip.getAppContext().getString(R.string.space_mins));
                        } else {
                            views.setTextViewText(R.id.widgetLowPredict, "");
                        }
                    } else {
                        views.setTextViewText(R.id.widgetLowPredict, xdrip.getAppContext().getString(R.string.low_predicted)+"!!!");
                    }
